...
class HttpCache extends Table with TableInfo<HttpCache, HttpCacheData> {
  ...
  late final GeneratedColumn<String> requestHeaders = GeneratedColumn<String>(
      'request_headers', aliasedName, false,
      type: DriftSqlType.string,
      requiredDuringInsert: true,
      $customConstraints: 'NOT NULL');
  ...
  HttpCacheData map(Map<String, dynamic> data, {String? tablePrefix}) {
    ...
    return HttpCacheData(
      ...
      requestHeaders: attachedDatabase.typeMapping.read(
          DriftSqlType.string, data['${effectivePrefix}request_headers'])!,
  ...
  @override
  List<String> get customConstraints => const ['UNIQUE(url, request_headers)'];
  @override
...
class HttpCacheData extends DataClass implements Insertable<HttpCacheData> {
  ...
  @override
  int get hashCode => Object.hash(id, url, headers, requestHeaders, body, date,
      maxAge, staleWhileRevalidate, staleIfError, immutable);
  @override
...
abstract class _$HttpGetCacheDatabase extends GeneratedDatabase {
  ...
  late final Index httpCacheIdx = Index('http_cache_idx',
      'CREATE UNIQUE INDEX http_cache_idx ON http_cache (url, request_headers, date, max_age, stale_if_error, stale_while_revalidate)');
  Future<List<HttpCacheData>> setHttpCache(
    ...
    return customWriteReturning(
        'INSERT OR REPLACE INTO http_cache (url, headers, request_headers, body, date, max_age, stale_while_revalidate, stale_if_error, immutable) VALUES (?1, ?2, ?3, ?4, ?5, ?6, ?7, ?8, ?9) RETURNING *',
        variables: [
          Variable<String>(url),
          Variable<String>(headers),
          Variable<String>(requestHeaders),
...
class $HttpCacheAnnotationComposer
  ...
  GeneratedColumn<String> get requestHeaders => $composableBuilder(
      column: $table.requestHeaders, builder: (column) => column);
